...
// PEM supports multiple type of panels in a single workspace, hence - panel
// could be an array type.
const workspaceSupportPanelId = (wsconfig, panelId) => {
  const panel = wsconfig.panel;
  const hasPanel = (panel) => panelId.indexOf(panel) >= 0;

  if (!_.isObject(panel)) return hasPanel(panel);

  return Object.values(panel).some(hasPanel);
};

export function WorkspaceProvider({children}) {
  const pgAdmin = usePgAdmin();
  const [currentWorkspace, setCurrentWorkspace] = useState(WORKSPACES.PEM_MANAGE);
  ...
  if (_.isUndefined(pgAdmin.Browser.docker.currentWorkspace)) {
    pgAdmin.Browser.docker.currentWorkspace = WORKSPACES.PEM_MANAGE;
  }
  ...
  */
  ...
  pgAdmin.Browser.getDockerHandler = (panelId, classicDocker)=>{
    ...
    const wsConfig = config.find((ws) => workspaceSupportPanelId(ws, panelId));
    if (wsConfig) {
  ...
  const onWorkspaceDisabled = ()=>{
    changeWorkspace(WORKSPACES.PEM_MANAGE);
  };
