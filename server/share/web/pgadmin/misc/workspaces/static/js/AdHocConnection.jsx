...
class AdHocConnectionSchema extends BaseUISchema {
  ...
  get baseFields() {
    ...
    return [
      {
        id: 'sid', label: gettext('Existing Server'), deps: ['connected', 'connection_refresh'],
        type: (state) => ({
          type: 'select',
      ...
      {
        id: 'server_name', label: gettext('Server Name'), type: 'text', noEmpty: false,
        deps: ['sid', 'connected'],
        disabled: (state) => state.sid, visible: false
      }, {
        ...
        canAdd: false, canEdit: false, canDelete: true,
      }, {
    ...
    try {
      ...
      if(!error.response) {
        pgAdmin.Browser.notifier.pgNotifier(
          ...
          gettext('Connection to PEM web server has been lost.')
        );
      } else {
