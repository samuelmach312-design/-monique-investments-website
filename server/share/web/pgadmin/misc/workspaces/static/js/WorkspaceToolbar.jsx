...
// Permissions are not need for PEM
...
import checkPrivilege from '../../../../browser/static/js/checkPrivilege';
    ...
    <StyledWorkspaceButton className={active ? 'active': ''} title={menuItem?.label??''}
      ...
      onClick={()=>{
        if(menuItem) {
          ...
          // pgAdmin Permissions not Required for PEM
          // // Check permission and call.
          // withCheckPermission(options, () => {
          //   changeWorkspace(value);
          // })();
          // Required for PEM
          checkPrivilege({privilege: props.privilege, label: props.title}, () => {
            changeWorkspace(value);
          });
        }
      }}
...
export default function WorkspaceToolbar() {
  ...
  return (
    <Root>
      <WorkspaceButton icon={<ManageHistoryIcon />} value={WORKSPACES.PEM_MANAGE} title={gettext('Enterprise Manager')} tooltipPlacement="right" />
      <WorkspaceButton icon={<AccountTreeRoundedIcon />} value={WORKSPACES.DEFAULT} title={gettext('Object Explorer')} tooltipPlacement="right" />
      <WorkspaceButton icon={<QueryToolIcon />} value={WORKSPACES.QUERY_TOOL} title={gettext('Query Tool Workspace')} tooltipPlacement="right" privilege='pem_query_tool'/>
      ...
      <WorkspaceButton icon={<SchemaDiffIcon />} value={WORKSPACES.SCHEMA_DIFF_TOOL} title={gettext('Schema Diff Workspace')} tooltipPlacement="right" privilege='pem_schema_diff_tool'/>
      <WorkspaceButton icon={<ConstructionIcon />} value={WORKSPACES.PEM_TOOLS} title={gettext('Enterprise Tools')} tooltipPlacement="right" />
      <Box marginTop="auto">
