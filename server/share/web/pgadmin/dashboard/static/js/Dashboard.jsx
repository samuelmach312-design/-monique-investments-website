...
import NestedTable from 'pem.charts/table/NestedTable.jsx';
import {
  getBarmanServerColumns,
  getBarmanBackupColumns,
  handleOpts,
} from './barman_servers/utils.js';
import BarmanChartFilters from './barman_servers/BarmanChartFilters.jsx';
import {INIT_STATES} from './barman_servers/constants.js';
...
const Root = styled('div')(({theme}) => ({
  ...
  '& .Dashboard-dashboardPanel': {
    ...
    '& .Dashboard-panelContent': {
      ...
      '& .Dashboard-barman-server': {
        overflowY: 'scroll',
        padding: theme.spacing(0, 1.375, 1.375),
        '& .chart-container': {
...
function Dashboard({
  ...
  const [barmanServerDetail, setBarmanServerDetail] = useState([]);
  const [barmanBackupDetail, setBarmanBackupDetail] = useState([]);
  const [nestedTableData, setNestedTableData] = useState(INIT_STATES.NESTED_TABLE_STATE);
  const [nestedTablesTabVal, setNestedTablesTabVal] = useState(0);

  const [selectedServer, setSelectedServer] = useState(INIT_STATES.ALL_SELECTED_STATE);
  const [selectedStatus, setSelectedStatus] = useState(INIT_STATES.ALL_SELECTED_STATE);
  const [selectedTimeframe, setSelectedTimeframe] = useState(INIT_STATES.TIMEFRAME_STATE);
  const [selectedDate, setSelectedDate] = useState(new Date().getTime());

  const [timelineData, setTimelineData] = useState([]);
  const [chartData, setChartData] = useState([]);
  const adjustedPositions = useRef([]);
  ...
  const barmanServer = treeNodeInfo?.barman_server ?? false;
  ...
  const fetchBarmanTableData = () => {
    const fetchData = (endpoint, setState) => {
      const transactionId = Date.now();
      api
        .get(
          `${pgAdmin.Browser.URL}barman_server/${barmanServer._id}/${endpoint}?_=${transactionId}`
        )
        .then((res) => {
          setState(res.data);
        })
        .catch((error) => {
          pgAdmin.Browser.notifier.alert(
            ...
            _.isUndefined(error.response)
              ? error.message
              : error.response.data.errormsg
          );
        });
    };
    fetchData('servers', setBarmanServerDetail);
    fetchData('backups', setBarmanBackupDetail);
  };

  const fetchBarmanChartData = () => {
    // Adding 1 day(86399000) to the selected date to include the selected date
    let url = `${pgAdmin.Browser.URL}barman_server/${barmanServer?._id}/stats/${selectedTimeframe?.value}/${selectedDate + 86399000}/`;
    if (selectedServer?.value !== 'all') {
      url += `${selectedServer?.value}`;
    }
    api
      .get(url)
      .then((res) => {
        let response = res?.data?.action.map((entry) => {
          const start = parseInt(entry.start_time);
          const end = entry.end_time ? parseInt(entry.end_time) : start + 1000;
          const duration = entry.duration ?? '00:00:01:000000';
          return {
            ...entry,
            end_time: String(end),
            duration,
        ...
        if (selectedStatus?.value !== 'all') {
          response = response.filter(
            (entry) => entry.status === selectedStatus?.value
          );
        }
        setChartData(response);

        const timelineArray = response.flatMap((entry) => [
          Math.floor(parseInt(entry.start_time) / 1000),
          Math.floor(parseInt(entry.end_time) / 1000),
        ]);

        const statusArray = timelineArray.map((_, index) =>
          index % 2 === 0 ? response[index / 2].status : null
        );

        const result = [[timelineArray, statusArray]];
        let timelineChartData = uPlot.join(
          result,
          result.map(() => [2, 2])
        );
  ...
  useEffect(() => {
    if (barmanServer) {
      fetchBarmanChartData();
    }
  }, [selectedDate, selectedServer, selectedStatus, selectedTimeframe]);
        ...
        if (mainTabVal == 1 || mainTabVal == 2 || mainTabVal == 3) {
          ...
          api({
            ...
            .then((res) => {
              const data = res.data;
              if(!data['ss_present']){
                setSsMsg(gettext('The system_stats extension is not installed. You can install the extension in a database using the "CREATE EXTENSION system_stats;" SQL command. Reload PEM once it is installed.'));
                setLdid(0);
              } else {
                setSsMsg('installed');
                setLdid(did);
  ...
  const BarmanServerNestedTableElement = renderNestedTable([
    gettext('Configuration'),
  ]);
  const BarmanBackupNestedTableElement = renderNestedTable([
  ...
  return (
    (<Root>
      {sid && serverConnected ? (
        ...
        <Box className='Dashboard-dashboardPanel'>
          <Box className='Dashboard-panelContent'>
            <div className='Dashboard-barman-server'>
              <ChartAccordion label='Barman Server Activities'>
                <BarmanChartFilters
                  barmanServerDetail={barmanServerDetail}
              ...
              <ChartAccordion label='Servers'>
                <PgTable
                  caveTable={false}
                  type='panel'
                  tableNoBorder={false}
                  columns={getBarmanServerColumns(setNestedTableData)}
                  variant='dashboardTable'
                  data={barmanServerDetail}
                  NestedTable={BarmanServerNestedTableElement}
                />
              </ChartAccordion>
              <ChartAccordion label='Backups'>
                <PgTable
                  caveTable={false}
                  type='panel'
                  tableNoBorder={false}
                  columns={getBarmanBackupColumns(setNestedTableData)}
                  variant='dashboardTable'
                  data={barmanBackupDetail}
                  NestedTable={BarmanBackupNestedTableElement}
                />
              </ChartAccordion>
            </div>
          </Box>
