...
from pgadmin.browser.server_groups.servers.pem import \
    PEMServerModule, PEMServerNode, PEMServerMenuItem
from . import api
...
blueprint = PEMServerModule(__name__)
...
class ServerNode(PGChildNodeView):
    ...
    def connect(self, gid, sid, is_qt=False, server=None):
        ...
        if 'password' not in data and (server.kerberos_conn is False or
            ...
            # PEM uses its own encryption module
...
SchemaDiffRegistry(blueprint.node_type, PEMServerNode)
PEMServerNode.register_node_view(blueprint)
