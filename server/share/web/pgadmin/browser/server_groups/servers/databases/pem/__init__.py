...
#
# Copyright (C) 2016 - 2025, The pgAdmin Development Team
...
from pgadmin.browser.server_groups.servers.databases.pem.utils import \
    SchemaRestriction
...
class PemDatabaseModule(CollectionNodeModule):
    _NODE_TYPE = 'database'
...
blueprint = PemDatabaseModule(__name__)


class PemDatabaseView(PGChildNodeView):
    ...
    def properties(self, gid, sid, did):
        ...
        result['schema_res'] = SchemaRestriction.get(sid, result['name'])
    ...
    @staticmethod
    def getdbservername(sid, pem_conn):
        # Get the server name to display in message from 'pem.server' table
        status, ret = pem_conn.execute_dict(
            "SELECT description FROM pem.server WHERE id = %(sid)s",
            {'sid': sid}
        )

        if not status:
            current_app.logger.error(
                "Failed to get database server name (#{0}).\nError: {1}".format
                (sid, ret)
            )
            sname = ''
        else:
            sname = ret['rows'][0]['description']

        return sname

    @pem_connection
    def connect(self, gid, sid, did, pem_conn=None):
        ...
        info_prefix = self.getdbservername(sid, pem_conn)
        if not info_prefix:
            info_prefix = '{0}'.format(conn.db)
        else:
            info_prefix = '{0}/{1}'.format(info_prefix, conn.db)
        ...
        return make_json_response(
            ...
            data={
                ...
                'connected': True,
                # Server name has been fetched for PEM separately.
                'info_prefix': info_prefix
            }
        )

    @pem_connection
    def disconnect(self, gid, sid, did, pem_conn=None):
    ...
    def create(self, gid, sid):
        ...
        SchemaRestriction.update(sid, response['name'], data)
    ...
    def update(self, gid, sid, did):
        ...
        PemDatabaseView._update_db_schema_res(data, did, sid)
    ...
    def delete(self, gid, sid, did=None):
        ...
        data = PemDatabaseView._get_req_data(did)
