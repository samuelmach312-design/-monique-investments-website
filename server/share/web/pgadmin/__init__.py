...
def create_app(app_name=None):
    ...
    if len(config.AUTHENTICATION_SOURCES) > 0:
        ...
        # This change is done to avoid selecting INTERNAL authentication
        # when user mistakenly keeps that the first option.
    ...
    # Initialize the PEM authentication methods.
    # Always call this only after 'security.init_app(..)' as PEM authentication
    # depends on the 'app.login_manager'.
    _pem.init_auth(app)
    ...
    def before_request():
        ...
        if config.SERVER_MODE and current_user.is_authenticated and \
            app.PGADMIN_EXTERNAL_AUTH_SOURCE not in [
                PEM_INTERNAL, KERBEROS, OAUTH2, WEBSERVER] and \
                current_app.keyManager.get() is None and \
                not current_user.has_token and \
    ...
    # PEM:: Initialize the REST APIs
    from pgadmin.pem.api.utils import init_api
    init_api(app)
...
def register_api_blueprint(api_blueprint):
    ...
    """
    import config
    PgAdmin.api_blueprints = []
    PgAdmin.api_blueprints.append(api_blueprint.import_name)
...
def register_api_views(api_blueprint):
    """
    This will look up for any token api view/s in ApiViewRegistry for given api
    ...
    from pgadmin.pem.api.utils import ApiViewRegistry
    try:
        apiviews = ApiViewRegistry.views[api_blueprint.name]
    except KeyError:
        return
    ...
    for apiview in apiviews:
        ...
        if 'GET' in methods:
            api_blueprint.add_url_rule(
                ...
                view_func=view_func, methods=['GET', ], is_token_url=True
            )
        ...
        if apiview.pk is not None and len(methods) > 0:
            api_blueprint.add_url_rule(
                ...
                view_func=view_func, methods=methods, is_token_url=True
            )


def register_api_routes(api_blueprint):
    """
    This will look up for any route/s in ApiViewRegistry for given api module
    ...
    from pgadmin.pem.api.utils import ApiViewRegistry
    try:
        apiroutes = ApiViewRegistry.routes[api_blueprint.name]
    except KeyError:
        return
