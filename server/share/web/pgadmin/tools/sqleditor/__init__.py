...
# for Query Tool privilege of PEM
queryToolRole = PEMRole(
    'pem_query_tool', gettext('Query Tool'),
    gettext('Query Tool'),
    gettext('Priviledge to access Query Tool functionalities')
)
...
def _connect(conn, **kwargs):
    ...
    if 'user' in kwargs and 'role' in kwargs:
        ...
        password = _pem.encrypt(
            kwargs['password']) if kwargs['password'] else None
