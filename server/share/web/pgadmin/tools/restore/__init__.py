...
# for Restore Tool privilege of PEM
restoreToolRole = PEMRole(
    'pem_restore_tool', _('Restore Tool'),
    _('Restore Tool role'),
    _('Priviledge to access Restore Tool functionalities')
)
    ...
    try:
        with open(path, "rb") as f:
            ...
            try:
                ...
                current_app.logger.warning(f"Security Alert: File {path} "
                                           f"is not valid UTF-8.")
                return False

            # --- SECURITY CHECK 2: Null Bytes ---
            # C-based tools (like psql) can behave unpredictably
            # with null bytes.
            if "\0" in text_data:
                current_app.logger.warning(f"Security Alert: File {path} "
                                           f"contains null bytes.")
                return False

            # --- SECURITY CHECK 3: Normalized Line Endings ---
            # We normalize all weird line endings (\r, \r\n, Form Feed) to \n
            # so we don't have to write a complex regex.
            # Note: \x0b (Vertical Tab) and \x0c (Form Feed) are
            # treated as breaks.
