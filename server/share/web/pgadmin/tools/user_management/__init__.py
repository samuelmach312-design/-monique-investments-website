...
def current_user_info():
    ...
    auth_source_manager = getattr(session, 'auth_source_manager', None)

    if auth_source_manager is None:
        # We support only one authentication method at a time.
        current_auth_source = config.AUTHENTICATION_SOURCES[0]
    else:
        current_auth_source = auth_source_manager['current_source']

    user_id = current_user.id
    # PEM does not have email here.
    # email = current_user.email.replace("'","\\'")
    # if current_user.email else ''
    pem_database_server_registration = 'true' if has_pem_role(
        'pem_database_server_registration') else 'false'
    username = current_user.username.replace("'","\\'") \
        if current_user.username else ''
    allow_save_password = 'true' if config.ALLOW_SAVE_PASSWORD and \
        session.get('allow_save_password', None) else 'false'
    allow_save_tunnel_password = 'true' if config.ALLOW_SAVE_TUNNEL_PASSWORD \
        and session.get('allow_save_password', None) else 'false'
    allow_auth_other_than_kerberos = getattr(
        config, 'ALLOW_DATABASE_CONNECTION_WITHOUT_KERBEROS', False
    )

    name = current_user.username.split('@')[0].replace("'","\\'") \
        if current_user.email else '<unknown>'
    pem_super_admin = 'true' if current_user.is_super_admin else 'false'
    is_pem_admin = 'true' if current_user.is_admin else 'false'
    is_admin = is_pem_admin
    ...
    return Response(
        response=render_template(
            ...
            allow_auth_other_than_kerberos='true'
            if allow_auth_other_than_kerberos else 'false',
