...
from pgadmin.utils.pem.server_group import ServerGroup
...
# for Schema Diff privilege of PEM
queryToolRole = PEMRole(
    'pem_schema_diff_tool', gettext('Schema Diff Tool'),
    gettext('Schema Diff Tool role'),
    gettext('Priviledge to access Schema Diff Tool functionalities')
)
...
def servers():
    ...
    try:
        ...
        for server in Server.all():
            ...
            server_group_data[server.server_group_name].append({
                "value": server.id,
                "label": server.name,
    ...
    return make_json_response(data=server_group_data)
