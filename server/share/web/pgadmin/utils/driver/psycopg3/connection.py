...
# PEM uses its own connection locker
def ConnectionLocker(kerberos_conn):
    from pgadmin.pem import _pem
    from flask import session

    return _pem.KerberosEnvLocker(
        session.get('KRB5CCNAME', None), kerberos_conn
    )
...
WHERE db.datname = current_database()""")
    ...
    def execute_post_connection_sql(self, cur, manager):
        ...
        errmsg = None
        if hasattr(manager, 'post_connection_sql') and \
                manager.post_connection_sql != '':
    ...
    def execute_on_server_as_csv(self, records=2000):
        ...
        current_app.logger.log(
            ...
            "{conn_id} (Query-id: {query_id}):\n{query}".format(
                pga_user=getattr(current_user, 'email', 'LoginProcess'),
    ...
    def execute_async(self, query, params=None, formatted_exception_msg=True,
        ...
        current_app.logger.log(
            ...
            "{query_id}):\n{query}".format(
                pga_user=getattr(current_user, 'username', 'LoginProcess'),
                db_user=self.conn.info.user,
                db_host=self.conn.info.host,
                db_name=self.conn.info.dbname,
                server_id=self.manager.sid,
