...
# PEM Specific helper functions
def not_found(errormsg=_('Not Found'), info=None):
    return make_json_response(
        status=404,
        success=0,
        errormsg=errormsg,
...
def conflict(errormsg=''):
    """Create a response with HTTP status code 409 - CONFLICT."""
    return make_json_response(
        status=409,
        success=0,
        errormsg=errormsg
...
def created(data, message):
    """Create a response with HTTP status code 201 - Created."""
    return make_json_response(
        status=201,
        success=1,
        info=message,
