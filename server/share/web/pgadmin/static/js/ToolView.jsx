...
import IDComponentMapping from '../../pem/static/js/Panels/mapping';
...
const getContent = (panelId, toolUrl, formParams, tabParams, restore, layoutDockerObj) => {
  if(tabParams.is_pem_tab){
    const Component = IDComponentMapping[panelId.replace(/-\d+$/, '')];
    layoutDockerObj.eventBus.registerListener(LAYOUT_EVENTS.CLOSING, (panelId)=>{
      layoutDockerObj.close(panelId, true);
    });
    if(Component) return <Component {...tabParams.tabProps}/>;
  }
  return (
...
export function getToolTabParams(panelId, toolUrl, formParams, tabParams, restore=false, layoutDockerObj=null) {
  ...
  return {
    id: panelId,
    title: panelId,
    content: getContent(panelId, toolUrl, formParams, tabParams, restore, layoutDockerObj),
    closable: true,
    manualClose: true,
    ...tabParams,
    cache: false,
