...
export class ManageTreeNodes {
  ...
  public addNode = (_parent: string, _path: string, _data: any) => new Promise((res) => {
    ...
    _data.is_shadow_node = Boolean(pgAdmin.Browser.Nodes?.[_data._type]?.is_shadow_node);
  ...
  public readNode = async (_path: string) => {
    ...
    if (_path == '/browser') {
      ...
      if (node.metadata.data._pid == null ) {
        ...
        url = _type + '/nodes/' + (_parent_url ? _parent_url + '/' : '');
      }
      else {
        url = node.metadata.data._type + '/children/' + (_parent_url ? _parent_url + '/' : '') + node.metadata.data._id;
      }
    ...
    while(_path != '/') {
      ...
      if(node.parentNode && node.parentNode.path == _parent) {
        if (node.parentNode.metadata.data !== null && !node.parentNode.metadata.data._type.includes('coll-'))
          ...
          } else if(!prevPushedNode?.metadata.data.is_shadow_node) {
            _parent_path.push(node.parentNode.metadata.data._id);
          }
      }
      _path = _parent;
      prevPushedNode = node.parentNode;
    }
