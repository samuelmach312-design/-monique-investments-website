...
import { ApplicationStateProvider, useApplicationState } from 'top/settings/static/ApplicationStateProvider';
...
export default function Layout({groups, noContextGroups, getLayoutInstance, layoutId, savedLayout, resetToTabPanel, enableToolEvents=false, isLayoutVisible=true, ...props}) {
  ...
  const loadTab = (tab)=>{
    ...
    if(!tabData && tab.metaData) {
      return LayoutDocker.getPanel(getToolTabParams(tab.id, tab.metaData.toolUrl, tab.metaData.formParams, tab.metaData.tabParams, tab.metaData?.restore, layoutDockerObj));
    }
    return tabData;
  };
