...
function AlertContent({ text, confirm, okLabel = gettext('OK'), cancelLabel = gettext('Cancel'), onOkClick, onCancelClick, okIcon = 'default', extraButtons }) {
  return (
      ...
      <Box className='Alert-footer'>
        ...
        }
        {
          extraButtons?.length ?
            ...
            :
            <PrimaryButton className='Alert-margin' startIcon={buttonIconMap[okIcon]} onClick={onOkClick} autoFocus={true} >{okLabel}</PrimaryButton>
        }
      </Box>
    </StyledBox>
  );
...
function confirm(title, text, onOkClick, onCancelClick, okLabel = gettext('Yes'), cancelLabel = gettext('No'), okIcon = 'default', extras = null) {
  // bind the modal provider before calling
  this.showModal(title, (closeModal) => {
    ...
    return (
      <AlertContent text={text} confirm onOkClick={onOkClickClose} onCancelClick={onCancelClickClose} okLabel={okLabel} cancelLabel={cancelLabel} extraButtons={extraButtons} okIcon={okIcon}/>
    );
  });
}
...
function ModalContainer({ id, title, content, dialogHeight, dialogWidth, onClose, fullScreen = false, isFullWidth = false, showFullScreen = false, isResizeable = false, minHeight = MIN_HEIGHT, minWidth = MIN_WIDTH, showTitle=true, showBackdrop=true }) {
  ...
  return (
    <StyleDialog
      ...
      slotProps={{
        backdrop: {
          sx: { display: showBackdrop ? 'block' : 'none' },
        },
      }}
    >
      { showTitle &&
