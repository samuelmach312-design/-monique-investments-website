...
import {
  ...
  InputTree, PlainString, InputColor
...
import {PEMMappedCellControl, PEMMappedFormControl} from './PEMMappedControl';


/* Control mapping for form view */
function MappedFormControlBase({
  id, type, state, onChange, className, inputRef, visible, onClick,
  ...
  const control = PEMMappedFormControl({
    type, value, id, onChange, className, inputRef, noLabel,
    onClick, withContainer, controlGridBasis, ...props
  });
  ...
  case 'button':
    return <FormButton
      name={name} value={value} className={className} onClick={onClick}
      {...props}
    />;
  case 'tree':
    return <InputTree
...
function MappedCellControlBase({
  ...
  const cellControl = PEMMappedCellControl({
    cell, value, id, name, optionsLoaded, onCellChange, reRenderRow,
    inputRef, ...props
  });

  if (cellControl) return cellControl;
...
const ALLOWED_PROPS_FIELD_COMMON = [
  ...
  'visible', 'autoFocus', 'className', 'optionsReloadBasis',
  'orientation', 'isvalidate', 'fields', 'radioType', 'hideBrowseButton',
  'btnName', 'hidden', 'withContainer', 'controlGridBasis', 'hasCheckbox',
  'treeData', 'labelTooltip', 'NodeComponent', 'onToggle', 'onCheckBoxClick',
  'handleChecked', 'suffixLabel'
];

const ALLOWED_PROPS_FIELD_FORM = [
  'type', 'onChange', 'state', 'noLabel', 'text','onClick', 'helpMessage'
];
...
export const MappedFormControl = ({
  ...
  newProps.onClick = ()=>{
    return origOnClick?.(state, schemaState, accessPath);
  };
