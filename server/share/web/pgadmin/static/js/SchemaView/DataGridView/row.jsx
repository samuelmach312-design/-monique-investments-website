...
export function DataGridRow({row, isResizing}) {
  ...
  const rowAccessPath = schemaState.accessPath(accessPath, row.id);
  ...
  features.current?.onRow({
    index: row.id, row, rowRef, classList, attributes, expandedRowContents,
    rowOptions, tableOptions: options
  });
  // rowId here needs to be changed to row.id
  let depsMap = [
    row.id, row.original?.id, isResizing, row?.getIsExpanded(),
