...
export function FormInput({ children, error, className, label, helpMessage, required, testcid, lid, withContainer=true, labelGridBasis=3, controlGridBasis=9, labelTooltip='', suffixLabel='', errorMessage='' }) {
    ...
    return (
      (<>
        ...
        <StyledGrid size={{ lg:controlGridBasis, md:controlGridBasis, sm:12, xs:12 }}>
          <FormControl error={Boolean(error)} fullWidth sx={suffixLabel?{ gap: '0.5rem', flexDirection: 'row', alignItems: 'center' }:{}}>
            {React.cloneElement(children, { cid, helpid })}
            {suffixLabel && <div>{suffixLabel}</div>}
          </FormControl>
  ...
  return (
    <StyledGrid container spacing={0} className={className} data-testid="form-input" sx={{ width: '100%' }}>
...
export function FormInputSQL({ hasError, required, label, className, helpMessage, testcid, value, controlProps, noLabel, withContainer, controlGridBasis, ...props }) {
  if (noLabel) {
...
export function FormInputDateTimePicker({ hasError, required, label, className, helpMessage, testcid, labelTooltip, withContainer, controlGridBasis, ...props }) {
  return (
...
export const InputText = forwardRef(({
  cid, helpid, readonly, disabled, value, onChange, controlProps, type, size, inputStyle, optionsLoaded, ...props }, ref) => {
  ...
  return (
    <OutlinedInput
      ...
      color="primary"
      fullWidth
      sx={{ width:controlProps?.width??'100%' }}
      size={size}
...
export function FormInputText({ hasError, required, label, className, helpMessage, testcid, labelTooltip, withContainer, controlGridBasis, suffixLabel, ...props }) {
  return (
    <FormInput required={required} label={label} error={hasError} className={className} helpMessage={helpMessage} testcid={testcid} labelTooltip={labelTooltip}
      withContainer={withContainer} controlGridBasis={controlGridBasis} suffixLabel={suffixLabel}>
...
export function FormInputFileSelect({
  hasError, required, label, className, helpMessage, testcid, labelTooltip, withContainer, controlGridBasis, ...props }) {
...
export function FormInputCheckbox({ hasError, required, label,
  className, helpMessage, testcid, labelTooltip, withContainer, controlGridBasis, ...props }) {
...
export function InputRadio({ helpid, value, onChange, controlProps, readonly, labelPlacement, disabled }) {
  ...
  return (
    <FormControlLabel
      ...
      label={controlProps.label}
      labelPlacement={labelPlacement}
      className={(readonly || disabled) ? 'Form-readOnlySwitch' : null}
    />
  );
}
...
export function FormInputToggle({ hasError, required, label,
  className, helpMessage, testcid, inputRef, labelTooltip, withContainer, controlGridBasis, ...props }) {
  return (
    <FormInput required={required} label={label} error={hasError} className={className} helpMessage={helpMessage} testcid={testcid} labelTooltip={labelTooltip}
      withContainer={withContainer} controlGridBasis={controlGridBasis}>
...
const customReactSelectStyles = (theme, readonly) => ({
  ...
  option: (provided, state) => {
    ...
    let color = 'inherit';
    if (state.isFocused) {
      bgColor = theme.otherVars.select.bgHover;
      color = theme.otherVars.select.fgHover;
    } else if (state.isSelected) {
      bgColor = theme.otherVars.select.bgSelected;
      color = theme.otherVars.select.fgSelected;
    }
    return {
      ...provided,
      padding: '0.5rem',
...
export function FormInputSelect({
  hasError, required, className, label, helpMessage, testcid, labelTooltip, withContainer, controlGridBasis, ...props }) {
  ...
  return (
    <FormInput required={required} label={label} error={hasError} className={className} helpMessage={helpMessage} testcid={testcid} labelTooltip={labelTooltip}
      withContainer={withContainer} controlGridBasis={controlGridBasis} errorMessage={selectError}>
...
export function PlainString({ controlProps, value }) {
  ...
  return  <Root>
    <div className="Form-plainstring">
      {controlProps?.renderAsHTML?HTMLReactParse(finalValue||''): finalValue}
    </div>
  </Root>;
