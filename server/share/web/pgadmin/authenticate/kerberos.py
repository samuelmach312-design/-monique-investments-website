...
class KerberosAuthentication(BaseAuthentication):
    ...
    def authenticate(self, frm):
        ...
        # PEM:: Now - create a connection for the pem backend database server
        #       using this kerberos ticket.
        if retval[0] is True:
            from pgadmin.pem import _pem
            return _pem.authenticate(
                email=session['__KRB5USER'], password=None, use_kerberos=True
            )
    ...
    def login(self, form):
        ...
        if user is None:
            user = User.query.filter_by(
                username=username, auth_source=KERBEROS
            ).first()
        ...
        current_app.logger.info(
            "Kerberos user {0} logged in.".format(username)
        )
