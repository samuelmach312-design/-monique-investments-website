...
SELECT
     ...
     */
    CASE
        WHEN c.type = 'CL' AND cr.midx IS NULL OR cr.tval IS NULL THEN 'HES'
        WHEN c.type = 'CL' AND cr.midx IS NOT NULL THEN 'HS'
        ELSE c.type
    END AS type,
    /*
     ...
     */
     CASE
        WHEN c.type = 'CL' AND (cr.midx IS NULL OR cr.tval IS NULL) THEN
            cr.historical * 24
        ELSE NULL
    END AS span,
    CASE
...
SELECT type, timeout,
  ...
  default_colors, span, espan, points, labels, downloadformat,
  name, did, lvl as level
FROM
(
    /*
