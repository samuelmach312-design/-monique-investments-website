...
SELECT
    ...
    (SELECT l.colors FROM pem.line_chart l WHERE l.cid = (%(cid)s)::integer) AS colors,
    c.labels AS labels,
...
SELECT
    ...
    WHEN (%(objid)s)::integer IS NOT NULL AND cfg.objid = (%(objid)s)::integer AND
        ...
        THEN 500::integer
    END AS lvl,
    cfg.reload, cfg.colors, cfg.span, cfg.espan, cfg.points, cfg.sortseq,
    CASE
...
SELECT type, timeout,
  ...
  name, showackalerts, did, lvl as level
FROM
(
    /*
