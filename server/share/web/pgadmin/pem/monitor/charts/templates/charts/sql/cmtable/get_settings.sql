...
SELECT
     ...
     */
    CASE
        WHEN c.type = 'CT' AND cr.midx IS NULL OR cr.tval IS NULL THEN 'HES'
        WHEN c.type = 'CT' AND cr.midx IS NOT NULL THEN 'HS'
        ELSE c.type
    END AS type,
    /*
     ...
     */
     CASE
        WHEN c.type = 'CT' AND (cr.midx IS NULL OR cr.tval IS NULL) THEN
            cr.historical * 24
        ELSE NULL
    END AS span,
    CASE
        WHEN c.type = 'CT' AND
            cr.midx IS NULL OR cr.tval IS NULL THEN
            cr.extrapolated * 24
        ELSE NULL
    END AS espan,
...
SELECT
    ...
    WHEN (%(objid)s)::integer IS NOT NULL AND cfg.objid = (%(objid)s)::integer AND
        ...
        THEN 500::integer
    END AS lvl,
    cfg.reload, cfg.span, cfg.espan, cfg.points, cfg.sortseq,
    CASE
...
SELECT type, timeout, span, espan, points, sortseq, labels,
downloadformat, name, showackalerts, did, lvl as level
FROM
(
    /*
    ...
    */
    ...
    COALESCE(x.span, c.span) AS span,
    CASE
