...
blueprint = ApiVersionModule(MODULE_NAME, __name__, url_prefix='/api/v3')
...
def index():
    ...
    return flask.make_response(
        flask.render_template('pem/rest_api.html',
                              url=url_for('v3_api.rest_api_json')
                              ),
        200, {'Content-Type': 'text/html'}
    )


@blueprint.route("/REST_API_v3.json", endpoint="rest_api_json")
@pgCSRFProtect.exempt
def rest_api_json():
    """REST API document JSON"""
    return flask.make_response(
        flask.render_template(
            'pem/REST_API_v3.json',
