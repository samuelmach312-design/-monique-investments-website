...
BEGIN
	...
	IF is_smtp_enabled THEN
		...
		FOR i in 1..COALESCE(array_upper(mail_group_id, 1), 0) LOOP
			...
			FOR tmp_row IN SELECT grp_to, grp_cc, grp_bcc, grp_from, (EXTRACT(EPOCH FROM time_from) + EXTRACT(TIMEZONE FROM time_from)) as time_from, (EXTRACT(EPOCH FROM time_to) + EXTRACT(TIMEZONE FROM time_to)) as time_to FROM pem.email_group_option WHERE gid = mail_group_id[i]
			LOOP
