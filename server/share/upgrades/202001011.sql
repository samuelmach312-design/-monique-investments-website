...
-- NOTE: THIS SQL SCRIPT WILL BE RESPONSIBLE FOR 7.13 RELEASE PEM SCHEMA UPGRADE

BEGIN TRANSACTION;

CREATE OR REPLACE FUNCTION pem.schema_version()
  RETURNS integer AS
'SELECT 202001011::integer;'
  LANGUAGE 'sql' IMMUTABLE;
COMMENT ON FUNCTION pem.schema_version() IS 'Returns the version number of the PEM schema';

-- Update existing entry and set connect timeout to 10 seconds instead of infinite
UPDATE pem.server_options SET connect_timeout=10;

-- If new server has been added, set connection timeout to 10 seconds
ALTER TABLE pem.server_options ALTER COLUMN connect_timeout SET DEFAULT 10;

-- JIRA: PEM-2614
-- Update the session info probe code to catch "idle in transaction (aborted)" state
-- (Update the probe code only for supported database version PG9.2 and above)
...
SELECT datname AS database_name, pid AS procpid, usename, client_addr, client_port, backend_start,
		...
		state like '%idle in transaction%' AS is_idle_in_transaction,
...
SELECT datname AS database_name, pid AS procpid, usename, client_addr, client_port, backend_start,
		...
		state = 'idle' AS is_idle, state like '%idle in transaction%' AS is_idle_in_transaction,
		query ilike $$VACUUM%$$ as is_vacuum,
		client_port IS NULL AND (query like $$autovacuum:%$$ OR query like $$VACUUM%$$) as is_autovacuum,
		now() AS capture_time, wait_event, wait_event_type, query, state, state_change FROM pg_catalog.pg_stat_activity$sql$
WHERE probe_id = (SELECT id from pem.probe WHERE internal_name = 'session_info')
AND server_version_id IN (10906, 11000, 11100, 11200, 20906, 21000, 21100, 21200);

-- JIRA: PEM-1145
-- Update the Alert error template to count alert which are enable
...
AND description = 'Number of alerts in an error state.';

-- JIRA: PEM-3050
-- Syntax error for last analyze server level alert template and modify display_value for table,schema,database level if analyze never run
...
where display_name = 'Last Analyze' and description ='Hours since last analyze on the table.';
...
where display_name = 'Last Analyze' and description ='Hours since last analyze on the schema.';
...
AND	  database_name = '${database_name}'
ORDER BY last_analyze DESC NULLS LAST LIMIT 1$sql$
where display_name = 'Last Analyze' and description ='Hours since last analyze on the database.';
...
WHERE	server_id = ${server_id}
ORDER BY last_analyze DESC NULLS LAST LIMIT 1$sql$
where display_name = 'Last Analyze' and description ='Hours since last analyze on the server.';

-- JIRA: PEM-2573
-- Historical span fix for all line charts
...
-- JIRA: PEM-1406
-- Modify SQL for Swap consumption to show the used swap memory instead of total swap memory
update pem.alert_template SET sql = $sql$
SELECT (total_swap_memory_mb - free_swap_memory_mb) AS used_swap_memory_mb
FROM pemdata.memory_usage
WHERE agent_id = ${agent_id};
$sql$
where display_name = 'Swap consumption';

-- JIRA: PEM-2794
-- Configuration parameters for SNMP v3
ALTER TABLE pem.config
ADD COLUMN options json DEFAULT NULL;

INSERT INTO pem.config (param, value, unit, datatype) VALUES ('snmp_security_name', '', '', 'string'); -- The SNMP security name
INSERT INTO pem.config (param, value, unit, datatype) VALUES ('snmp_security_engine_id', '', '', 'string'); -- The SNMP security engine id
INSERT INTO pem.config (param, value, unit, datatype, options) VALUES ('snmp_security_level', '1', '', 'enum', '[{"label": "NOAUTH_NOPRIV", "value": "1"}, { "label": "AUTH_NOPRIV", "value": "2"}, {"label": "AUTH_PRIV", "value": "3" }]'); -- The SNMP security level
INSERT INTO pem.config (param, value, unit, datatype) VALUES ('snmp_context_name', '', '', 'string'); -- The SNMP context name
INSERT INTO pem.config (param, value, unit, datatype) VALUES ('snmp_context_engine_id', '', '', 'string'); -- The SNMP context engine id
INSERT INTO pem.config (param, value, unit, datatype, options) VALUES ('snmp_authentication_protocol', '1', '', 'enum', '[{"label": "NONE", "value": "1"}, { "label": "HMACMD5", "value": "2"}, {"label": "HMACSHA", "value": "3" }]'); -- The SNMP authentication protocol
INSERT INTO pem.config (param, value, unit, datatype, options) VALUES ('snmp_privacy_protocol', '1', '', 'enum', '[{"label": "NONE", "value": "1"}, { "label": "DES", "value": "2"}, {"label": "AES128", "value": "4" }, {"label": "IDEA", "value": "9"}, { "label": "AES192", "value": "20"}, { "label": "AES256", "value": "21"}, {"label": "3DESEDE", "value": "3" }]'); -- The SNMP privacy protocol
INSERT INTO pem.config (param, value, unit, datatype) VALUES ('snmp_authentication_password', NULL, '', 'password'); -- The SNMP authentication password
INSERT INTO pem.config (param, value, unit, datatype) VALUES ('snmp_privacy_password', NULL, '', 'password'); -- The SNMP privacy password

-- JIRA: PEM-3081
-- Modify condition for fetching avail agents, it was wrongly checking for role 'pem_super_user' it should be
-- 'pem_super_admin'
...
-- PEM-3139 - RLS policy was not updated properly for "pem.job" table during the PEM upgrade. [Case #937823]
DO $$
DECLARE
...
-- PEM-2934
-- The modified function has been missed in PEM 7.12
-- Need to drop and recreate function while upgrading from PEM 7.12 to PEM 7.13
DROP FUNCTION IF EXISTS pem.schedule_bart_init_jobs(integer, integer, integer);
