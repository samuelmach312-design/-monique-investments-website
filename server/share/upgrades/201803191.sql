...
UPDATE pem.alert_template SET info_sql = $sql$SELECT srv.description || ' (' || srv.server || ')' AS "Master server",
...
AND lag_MB ${comparison_operator} '${threshold_value}'::numeric;$sql$
  WHERE display_name = 'Standby servers lag behind the master by size(MB)' AND object_type = 200;

GRANT UPDATE(alert_blackout) ON pem.server TO pem_config_alert;
GRANT UPDATE(alert_blackout) ON pem.agent TO pem_config_alert;
