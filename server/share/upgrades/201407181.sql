...
CREATE OR REPLACE FUNCTION pem.schema_version()
  RETURNS integer AS
'SELECT 201407181::integer;'
  LANGUAGE 'sql' IMMUTABLE;

ALTER TABLE pem.log_configuration ADD COLUMN update_log_directory boolean NOT NULL DEFAULT false;
ALTER TABLE pem.audit_configuration ADD COLUMN update_audit_directory boolean NOT NULL DEFAULT false;
...
WHERE probe_id = $1::int4 AND (internal_name || ''_pit'') = $2::text AND is_graphable AND NOT pit_by_default AND calculate_pit'
	...
	v_aggqry := ARRAY[
'SELECT $6::int2 o_idx, $7::text o_label, to_timestamp((dn * EXTRACT(EPOCH FROM $2::interval)) + EXTRACT(EPOCH FROM $3::timestamptz)) o_aggtime, val::numeric o_aggval
FROM (SELECT
		unnest(CASE WHEN ((t1.dn = t2.dn) OR (t1.dn < 0 AND t2.dn < 0)) THEN ARRAY[t1.dn]
		WHEN t2.dn IS NULL AND t1.dn <= 0 THEN ARRAY(SELECT g FROM generate_series(0, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
		WHEN t2.dn IS NULL AND t1.dn < floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint THEN ARRAY(SELECT g FROM generate_series(t1.dn::bigint, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
		WHEN t1.dn != t2.dn AND t1.dn < 0 AND t2.dn > 0 THEN ARRAY(SELECT g FROM generate_series(0, (t2.dn - 1)::bigint, 1) g)
		WHEN t1.dn != t2.dn THEN ARRAY(SELECT g FROM generate_series(t1.dn::bigint, (t2.dn - 1)::bigint, 1) g)
		END) dn,
		t1.val val,
		unnest(CASE WHEN (t1.dn < 0 AND t2.dn < 0) THEN ARRAY[1::bigint]
		WHEN (t1.dn = t2.dn AND t1.dn > 0) THEN ARRAY[t1.pidx::bigint]
		WHEN t2.dn IS NULL AND t1.dn <= 0 THEN ARRAY(SELECT t1.pidx::bigint FROM generate_series(0, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
		WHEN t2.dn IS NULL AND t1.dn < floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint - 1 THEN t1.pidx::bigint || ARRAY(SELECT 1::bigint FROM generate_series((t1.dn + 1)::bigint, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint - 1, 1) g)
		WHEN t1.dn != t2.dn AND t1.dn < 0 AND t2.dn > 0 THEN ARRAY(SELECT 1::bigint FROM generate_series(0, (t2.dn - 1)::bigint, 1) g)
		WHEN t1.dn != t2.dn AND t2.dn - t1.dn > 1 THEN t1.pidx::bigint ||  ARRAY(SELECT 1::bigint FROM generate_series((t1.dn + 1)::bigint, (t2.dn - 1)::bigint, 1) g)
		WHEN t1.dn != t2.dn THEN ARRAY[t1.pidx::bigint]
		END) npidx
	FROM
		dtbl t1
...
WHERE npidx = 1
ORDER BY dn',
'SELECT $6::int2 o_idx, $7::text o_label, to_timestamp((dn * EXTRACT(EPOCH FROM $2::interval)) + EXTRACT(EPOCH FROM $3::timestamptz)) o_aggtime, max(val::numeric) o_aggval
FROM (SELECT
		unnest(CASE WHEN ((t1.dn = t2.dn) OR (t1.dn < 0 AND t2.dn < 0)) THEN ARRAY[t1.dn]
		WHEN t2.dn IS NULL AND t1.dn <= 0 THEN ARRAY(SELECT g FROM generate_series(0, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
		WHEN t2.dn IS NULL AND t1.dn < floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint THEN ARRAY(SELECT g FROM generate_series(t1.dn::bigint, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
	...
	FROM
		dtbl t1
		LEFT JOIN dtbl t2 ON (t1.rn = t2.rn - 1)) tGROUP BY dn
ORDER BY dn',
'SELECT $6::int2 o_idx, $7::text o_label, to_timestamp((dn * EXTRACT(EPOCH FROM $2::interval)) + EXTRACT(EPOCH FROM $3::timestamptz)) o_aggtime, min(val::numeric) o_aggval
FROM (SELECT
		unnest(CASE WHEN ((t1.dn = t2.dn) OR (t1.dn < 0 AND t2.dn < 0)) THEN ARRAY[t1.dn]
		WHEN t2.dn IS NULL AND t1.dn <= 0 THEN ARRAY(SELECT g FROM generate_series(0, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
		WHEN t2.dn IS NULL AND t1.dn < floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint THEN ARRAY(SELECT g FROM generate_series(t1.dn::bigint, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
...
GROUP BY dn
ORDER BY dn',
'SELECT $6::int2 o_idx, $7::text o_label, to_timestamp((dn * EXTRACT(EPOCH FROM $2::interval)) + EXTRACT(EPOCH FROM $3::timestamptz)) o_aggtime, sum(val::numeric * cnt) / sum(cnt) o_aggval
FROM (SELECT
		unnest(CASE WHEN ((t1.dn = t2.dn) OR (t1.dn < 0 AND t2.dn < 0)) THEN ARRAY[t1.dn]
		WHEN t2.dn IS NULL AND t1.dn <= 0 THEN ARRAY(SELECT g FROM generate_series(0, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
		WHEN t2.dn IS NULL AND t1.dn < floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint THEN ARRAY(SELECT g FROM generate_series(t1.dn::bigint, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
		...
		t1.val val,
		unnest(CASE WHEN (t1.dn < 0 AND t2.dn < 0) THEN ARRAY[1::bigint]
		WHEN (t1.dn = t2.dn AND t1.dn > 0) THEN ARRAY[(t2.pn - t1.pn)::bigint]
		WHEN t2.dn IS NULL AND t1.dn <= 0 THEN ARRAY(SELECT 1::bigint FROM generate_series(0, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint - 1, 1) g)
		WHEN t2.dn IS NULL AND t1.dn < floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint - 1 THEN ceil((((t1.dn + 1) * EXTRACT(EPOCH FROM $2::interval)) + EXTRACT(EPOCH FROM $3::timestamptz) - EXTRACT(EPOCH FROM t1.rt)) / EXTRACT(EPOCH FROM $1::interval))::bigint || ARRAY(SELECT 1::bigint FROM generate_series((t1.dn + 1)::bigint, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint - 1, 1) g)
		WHEN t1.dn != t2.dn AND t1.dn < 0 AND t2.dn > 0 THEN ARRAY(SELECT 1::bigint FROM generate_series(0, (t2.dn - 1)::bigint, 1) g)
		WHEN t1.dn != t2.dn AND t2.dn - t1.dn > 1 THEN ceil((((t1.dn + 1) * EXTRACT(EPOCH FROM $2::interval)) + EXTRACT(EPOCH FROM $3::timestamptz) - EXTRACT(EPOCH FROM t1.rt)) / EXTRACT(EPOCH FROM $1::interval))::bigint ||  ARRAY(SELECT 1::bigint FROM generate_series((t1.dn + 1)::bigint, (t2.dn - 1)::bigint, 1) g)
		WHEN t1.dn != t2.dn THEN ARRAY[ceil((((t1.dn + 1) * EXTRACT(EPOCH FROM $2::interval)) + EXTRACT(EPOCH FROM $3::timestamptz) - EXTRACT(EPOCH FROM t1.rt)) / EXTRACT(EPOCH FROM $1::interval))::bigint]
		END) cnt
	FROM
		dtbl t1
		LEFT JOIN dtbl t2 ON (t1.rn = t2.rn - 1)) t
...
ORDER BY dn'];
	...
	IF v_maxpoints IS NULL OR v_maxpoints < 1 THEN
		v_maxpoints := 300;
	END IF;
...
WITH dtbl AS (
	SELECT
		...
		dense_rank() OVER (PARTITION BY floor(dt / (EXTRACT(EPOCH FROM $2::interval))) ORDER BY floor(dt / (EXTRACT(EPOCH FROM $1::interval))) DESC) pidx,
		row_number() OVER (ORDER BY rt) rn
	FROM (
		SELECT
			EXTRACT(EPOCH FROM (ph.recorded_time - $3::timestamptz)) dt,
			ph.recorded_time rt, COALESCE(' || pg_catalog.quote_ident(v_t_op[4]) || ', 0) val
		FROM
			pemhistory.' || pg_catalog.quote_ident(v_t_op[3]) || ' ph
		WHERE ' || v_t_op[6] || ' AND ph.recorded_time >= $4::timestamptz AND ph.recorded_time <= $5::timestamptz) tbl
)
';
		...
		IF v_e_time > v_c_time THEN
			...
			IF v_cnt = 1 OR v_corr IS NULL OR v_value IS NULL OR v_s_time > v_c_time THEN
				IF (v_cnt = 1 AND v_value IS NOT NULL) OR
					...
					RETURN NEXT;
					RETURN QUERY EXECUTE 'SELECT $1::int2 o_idx, $2::text o_lbl, g o_aggtime, $3::numeric o_aggval FROM generate_series($4::timestamptz, $5::timestamptz, $6::interval) g' USING v_t_op[1], v_t_op[2], v_value, v_s_time, v_e_time, v_aggspan;
				ELSIF (v_corr IS NOT NULL AND v_value IS NOT NULL AND v_slope IS NOT NULL AND v_intercept IS NOT NULL) THEN
					...
					RETURN NEXT;
					RETURN QUERY EXECUTE 'SELECT $1::int2 o_idx, $2::text o_lbl, g o_aggtime, ($3::numeric * EXTRACT(EPOCH FROM g)) + $4::numeric o_aggval FROM generate_series($5::timestamptz, $6::timestamptz, $7::interval) g' USING v_t_op[1], v_t_op[2], v_slope, v_intercept, v_s_time, v_e_time, v_aggspan;
				ELSE
					o_idx := -1;
					o_aggtime := NULL;
					o_aggval  := NULL;
			...
			ELSE
				...
				IF v_minfreq > v_aggspan THEN
					v_aggspan := v_minfreq;
				END IF;
				SELECT floor(EXTRACT(EPOCH FROM (v_c_time - v_s_time)) / EXTRACT(EPOCH FROM v_aggspan))::bigint INTO v_maxpoints;
			END IF;
			...
			WHEN 'F' THEN
				...
				RETURN QUERY EXECUTE v_qry || v_aggqry[0] USING v_t_op[9], v_aggspan, v_s_time, v_t_op[10], v_c_time, v_t_op[1], v_t_op[2];
			WHEN 'M' THEN
				RETURN QUERY EXECUTE v_qry || v_aggqry[1] USING v_t_op[9], v_aggspan, v_s_time, v_t_op[10], v_c_time, v_t_op[1], v_t_op[2];
			WHEN 'm' THEN
				RETURN QUERY EXECUTE v_qry || v_aggqry[2] USING v_t_op[9], v_aggspan, v_s_time, v_t_op[10], v_c_time, v_t_op[1], v_t_op[2];
			ELSE
				RETURN QUERY EXECUTE v_qry || v_aggqry[3] USING v_t_op[9], v_aggspan, v_s_time, v_t_op[10], v_c_time, v_t_op[1], v_t_op[2];
			END CASE;

			IF v_e_time > v_c_time THEN
				SELECT to_timestamp(((v_maxpoints + 1) * EXTRACT(EPOCH FROM v_aggspan)) + EXTRACT(EPOCH FROM v_s_time)) INTO v_m_s_time;
		...
		ELSE
			...
			IF v_minfreq > v_aggspan THEN
				v_aggspan := v_minfreq;
				v_maxpoints := floor(EXTRACT(EPOCH FROM (v_c_time - v_s_time) / v_aggspan))::bigint;
			END IF;
			...
			WHEN 'F' THEN
				-- frequncy, span, st, tst, et, pos, lbl
				RETURN QUERY EXECUTE v_qry || v_aggqry[0] USING v_t_op[9], v_aggspan, v_s_time, v_t_op[10], v_e_time, v_t_op[1], v_t_op[2];
			WHEN 'M' THEN
				RETURN QUERY EXECUTE v_qry || v_aggqry[1] USING v_t_op[9], v_aggspan, v_s_time, v_t_op[10], v_e_time, v_t_op[1], v_t_op[2];
			WHEN 'm' THEN
				RETURN QUERY EXECUTE v_qry || v_aggqry[2] USING v_t_op[9], v_aggspan, v_s_time, v_t_op[10], v_e_time, v_t_op[1], v_t_op[2];
			ELSE
				RETURN QUERY EXECUTE v_qry || v_aggqry[3] USING v_t_op[9], v_aggspan, v_s_time, v_t_op[10], v_e_time, v_t_op[1], v_t_op[2];
			END CASE;
		END IF;
	ELSIF NOT (p_stime IS NOT NULL AND p_etime IS NOT NULL) AND v_e_span IS NOT NULL AND v_e_span >= '1 seconds'::interval THEN
		v_e_time := v_c_time + v_e_span;
		v_aggspan := ((v_e_time - v_s_time) / (v_maxpoints - 1))::interval;
		IF v_minfreq > v_aggspan THEN
			v_aggspan := v_minfreq;
			SELECT floor(EXTRACT(EPOCH FROM (v_e_time - v_s_time)) / EXTRACT(EPOCH FROM v_aggspan))::bigint INTO v_maxpoints;
		END IF;
	END IF;
	...
	IF v_t_op IS NOT NULL AND v_s_time <= v_c_time AND v_e_time > v_c_time THEN
		o_idx := -1;
		o_aggtime := v_c_time;
		o_aggval  := NULL;
		o_label :=  '115';
...
';
		...
		IF v_e_time > v_c_time THEN
			...
			--
			EXECUTE 'SELECT regr_slope(' || pg_catalog.quote_ident(v_m_ops[v_pos][4]) || '::numeric, EXTRACT(EPOCH FROM recorded_time)), regr_intercept(' || pg_catalog.quote_ident(v_m_ops[v_pos][4]) || '::numeric, EXTRACT(EPOCH FROM recorded_time)), corr(' || pg_catalog.quote_ident(v_m_ops[v_pos][4]) || '::numeric, EXTRACT(EPOCH FROM recorded_time)), count(*), (SELECT ' || pg_catalog.quote_ident(v_m_ops[v_pos][4]) || '::numeric FROM pemhistory.' || pg_catalog.quote_ident(v_m_ops[v_pos][3]) || ' WHERE ' || v_m_ops[v_pos][6] ||' ORDER BY recorded_time DESC LIMIT 1) FROM pemhistory.' || pg_catalog.quote_ident(v_m_ops[v_pos][3]) || ' WHERE ' || v_m_ops[v_pos][6]
			INTO v_slope, v_intercept, v_corr, v_cnt, v_value;
			...
			IF v_cnt = 1 OR v_corr IS NULL OR v_value IS NULL OR v_s_time > v_c_time THEN
				...
				ELSIF (v_corr IS NOT NULL AND v_value IS NOT NULL AND v_slope IS NOT NULL AND v_intercept IS NOT NULL) THEN
					RETURN QUERY EXECUTE 'SELECT $1::int2 o_idx, $2::text o_lbl, g o_aggtime, ($3::numeric * EXTRACT(EPOCH FROM g)) + $4::numeric o_aggval FROM generate_series($5::timestamptz, $6::timestamptz, $7::interval) g' USING v_m_ops[v_pos][1], v_m_ops[v_pos][2], v_slope, v_intercept, v_s_time, v_e_time, v_aggspan;
				ELSE
					v_m_e_time := v_c_time;
					RETURN;
				END IF;
			...
			IF v_s_time <= v_c_time THEN
				...
				WHEN 'F' THEN
					-- frequncy, span, st, tst, et, pos, lbl
					RETURN QUERY EXECUTE v_qry || v_aggqry[0] USING v_m_ops[v_pos][9], v_aggspan, v_s_time, v_m_ops[v_pos][10], v_c_time, v_m_ops[v_pos][1], v_m_ops[v_pos][2];
				WHEN 'M' THEN
					RETURN QUERY EXECUTE v_qry || v_aggqry[1] USING v_m_ops[v_pos][9], v_aggspan, v_s_time, v_m_ops[v_pos][10], v_c_time, v_m_ops[v_pos][1], v_m_ops[v_pos][2];
				WHEN 'm' THEN
					RETURN QUERY EXECUTE v_qry || v_aggqry[2] USING v_m_ops[v_pos][9], v_aggspan, v_s_time, v_m_ops[v_pos][10], v_c_time, v_m_ops[v_pos][1], v_m_ops[v_pos][2];
				ELSE
					RETURN QUERY EXECUTE v_qry || v_aggqry[3] USING v_m_ops[v_pos][9], v_aggspan, v_s_time, v_m_ops[v_pos][10], v_c_time, v_m_ops[v_pos][1], v_m_ops[v_pos][2];
				END CASE;
			END IF;

			IF v_m_e_time > v_c_time THEN
				SELECT to_timestamp(((v_maxpoints + 1) * EXTRACT(EPOCH FROM v_aggspan)) + EXTRACT(EPOCH FROM v_s_time)) INTO v_m_s_time;
				RETURN QUERY EXECUTE 'SELECT $1::int2 o_idx, $2::text o_lbl, g o_aggtime, (($3::numeric * EXTRACT(EPOCH FROM g)) + $4::numeric)::numeric o_aggval FROM generate_series($5::timestamptz, $6::timestamptz, $7::interval) g' USING v_m_ops[v_pos][1], v_m_ops[v_pos][2], v_slope, v_intercept, v_m_s_time, v_m_e_time, v_aggspan;
			END IF;
		ELSE
			-- 1. pos, 2. label, 3. probe_tbl, 4. probe_col, 5. agg, 6. condition, 7. groupon, 8. percentage_unit, 9. freq, 10. min_time
			CASE v_m_ops[v_pos][5]
			WHEN 'F' THEN
				-- frequncy, span, st, tst, et, pos, lbl
				RETURN QUERY EXECUTE v_qry || v_aggqry[0] USING v_m_ops[v_pos][9], v_aggspan, v_s_time, v_m_ops[v_pos][10], v_e_time, v_m_ops[v_pos][1], v_m_ops[v_pos][2];
			WHEN 'M' THEN
				RETURN QUERY EXECUTE v_qry || v_aggqry[1] USING v_m_ops[v_pos][9], v_aggspan, v_s_time, v_m_ops[v_pos][10], v_e_time, v_m_ops[v_pos][1], v_m_ops[v_pos][2];
			WHEN 'm' THEN
				RETURN QUERY EXECUTE v_qry || v_aggqry[2] USING v_m_ops[v_pos][9], v_aggspan, v_s_time, v_m_ops[v_pos][10], v_e_time, v_m_ops[v_pos][1], v_m_ops[v_pos][2];
			ELSE
				RETURN QUERY EXECUTE v_qry || v_aggqry[3] USING v_m_ops[v_pos][9], v_aggspan, v_s_time, v_m_ops[v_pos][10], v_e_time, v_m_ops[v_pos][1], v_m_ops[v_pos][2];
			END CASE;
		END IF;
	END LOOP;
