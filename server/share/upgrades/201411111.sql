...
-- Fixes #33611
-- Remove any custom probes, which are having the collection method as 'Batch/Shell Script' and targeted on non-agent level
-- We do support the custom probes with collection method - 'Batch/Shell script' only for the agent level.
UPDATE pem.probe SET deleted = true WHERE NOT is_system_probe AND collection_method = 'b' AND target_type_id > 100;
