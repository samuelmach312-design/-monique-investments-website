...
CREATE OR REPLACE FUNCTION pem.schema_version()
  RETURNS integer AS
'SELECT 201407211::integer;'
  LANGUAGE 'sql' IMMUTABLE;

UPDATE pem.chart SET labels = ARRAY['Session Id', 'User Name', 'Source', 'Database Name', 'Waiting', 'Backend Start', 'Transaction Start', 'Query Start', 'Memory Usage', 'Swap Usage', 'CPU Usage', 'I/O Reads (#bytes)', 'I/O Writes (#bytes)'] WHERE id = 62;
UPDATE pem.chart_func SET func = E'
	...
	SELECT
		...
		memory_usage_mb AS "Memory Usage",
		swap_usage_mb AS "Swap Usage",
		cpu_usage AS "CPU Usage",
		io_read_bytes AS "I/O Reads (bytes)",
		io_write_bytes AS "I/O Writes (bytes)"
	FROM
...
GROUP BY dn
ORDER BY dn',
'SELECT $6::int2 o_idx, $7::text o_label, to_timestamp((dn * EXTRACT(EPOCH FROM $2::interval)) + EXTRACT(EPOCH FROM $3::timestamptz)) o_aggtime, (sum(val::numeric * cnt) / sum(cnt))::numeric o_aggval
FROM (SELECT
		unnest(CASE WHEN ((t1.dn = t2.dn) OR (t1.dn < 0 AND t2.dn < 0)) THEN ARRAY[t1.dn]
		WHEN t2.dn IS NULL AND t1.dn <= 0 THEN ARRAY(SELECT g FROM generate_series(0, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
		WHEN t2.dn IS NULL AND t1.dn < floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint THEN ARRAY(SELECT g FROM generate_series(t1.dn::bigint, floor(EXTRACT (EPOCH FROM $5::timestamptz - $3::timestamptz) / EXTRACT(EPOCH FROM $2::interval))::bigint, 1) g)
...
';
		...
		IF v_e_time > v_c_time THEN
			...
			IF v_cnt = 1 OR v_corr IS NULL OR v_value IS NULL OR v_s_time > v_c_time THEN
				...
				ELSIF (v_corr IS NOT NULL AND v_value IS NOT NULL AND v_slope IS NOT NULL AND v_intercept IS NOT NULL) THEN
					...
					RETURN NEXT;
					RETURN QUERY EXECUTE 'SELECT $1::int2 o_idx, $2::text o_lbl, g o_aggtime, (($3::numeric * EXTRACT(EPOCH FROM g)) + $4::numeric)::numeric o_aggval FROM generate_series($5::timestamptz, $6::timestamptz, $7::interval) g' USING v_t_op[1], v_t_op[2], v_slope, v_intercept, v_s_time, v_e_time, v_aggspan;
				ELSE
					o_idx := -1;
					o_aggtime := NULL;
					o_aggval  := NULL;
			...
			IF v_e_time > v_c_time THEN
				...
				END IF;
				RETURN QUERY EXECUTE 'SELECT $1::int2 o_idx, $2::text o_lbl, g::timestamptz o_aggtime, (($3::numeric * EXTRACT(EPOCH FROM g)) + $4::numeric)::numeric o_aggval FROM generate_series($5::timestamptz, $6::timestamptz, $7::interval) g' USING v_t_op[1], v_t_op[2], v_slope, v_intercept, v_m_s_time, v_e_time, v_aggspan;
			END IF;
	...
	IF v_t_op IS NULL AND v_s_time <= v_c_time AND v_e_time > v_c_time THEN
		o_idx := -1;
		o_aggtime := v_c_time;
		o_aggval  := NULL;
		o_label :=  '115';
...
WHERE
	...
	recorded_time >= $3::timestamptz AND
	recorded_time <= $4::timestamptz
ORDER BY recorded_time'
