...
CREATE OR REPLACE FUNCTION pem.schema_version()
  RETURNS integer AS
'SELECT 201903211::integer;'
  LANGUAGE 'sql' IMMUTABLE;
COMMENT ON FUNCTION pem.schema_version() IS
 'Returns the version number of the PEM schema';

/*
 * To fix the issue raised by customer on Case# 853549, Where they were seeing
 * the lock info from the user which does not exists on that server.
 */
...
SET func = E'
	...
	FROM
	   ...
	   pemdata.session_info psi ON ( pli.procpid = psi.procpid and pli.server_id = psi.server_id )
