...
BEGIN
	...
	LOOP
		subquery = E'SELECT string_to_array(ind_keys::text, '' '') AS index_keys FROM pemdata.oc_index WHERE table_name = ' || pg_catalog.quote_literal(row.fktab) || ' AND schema_name = ' || pg_catalog.quote_literal(row.schema_name) || ';';
