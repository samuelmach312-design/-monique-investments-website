...
';
		...
		IF v_e_time > v_c_time THEN
			...
			IF v_s_time <= v_c_time THEN
				...
				ELSE
					RETURN QUERY EXECUTE replace(replace(replace(v_aggqry[4], '@@COLUMN@@', pg_catalog.quote_ident(v_m_ops[v_pos][4])), '@@TABLE@@', pg_catalog.quote_ident(v_m_ops[v_pos][3])), '@@CONDITION@@', v_m_ops[v_pos][6])  USING v_aggspan, v_s_time, v_m_ops[v_pos][10], v_c_time, v_m_ops[v_pos][1], v_m_ops[v_pos][2], v_maxpoints, v_t_op[9];
				END CASE;
			END IF;
